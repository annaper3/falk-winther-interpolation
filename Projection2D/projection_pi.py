...
def projection_pi(f, curl_f, mesh):
    ...
    #Elements
    RT = FiniteElement('RT',triangle,1) #Raviart-Thomas
    NE = FiniteElement('N1curl',triangle,1) #Nedelec
    LA = FiniteElement('Lagrange',triangle,1) #Lagrange
    DG = FiniteElement('DG',triangle,0) #DG
    ...
    for E in edges(mesh):
        ...
        for i, node in enumerate(vertices(E)):
            v_cells = node.entities(2) #get neighbouring cells to node
            for c in v_cells:
        ...
        bc_RT = DirichletBC(W.sub(0),Constant((0,0)), DomainBoundary())
        bc_LA = DirichletBC(W.sub(1),Constant((0)), DomainBoundary())
